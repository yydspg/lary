...
public class RoomServiceImpl extends ServiceImpl<RoomMapper, Room> implements RoomService {
    ...
    public ResponsePair<JoinLiveVO> join(long toUid,long sid, String ip) {
        ...
        UserCache data = userCacheComponent.getData(uid);
        data.setRid(live.getRid());
        userCacheComponent.setData(uid,data);
    ...
    public ResponsePair<DownLiveVO> end() {
        ...
        StreamRecord record = streamRecordService.lambdaQuery()
                ...
                .one();
        if (record == null) {
            log.error("internal error,no stream record found,rid:{}",rid);
        ...
        Room room = lambdaQuery()
                ...
                .one();
        if (room == null) {
            log.error("internal error,no room found,rid:{}",rid);
...
//            long event = eventService.begin(downLiveEventDTO);
            streamRecordService.lambdaUpdate()
                    ...
                    .set(StreamRecord::getNewFansNum, recordCache.getNewFansNum())
                            .update();
            streamRecordService.lambdaUpdate()
                    .eq(StreamRecord::getSid, liveCache.getStreamId())
    ...
    public ResponsePair<GoLiveVO> go(String ip, GoLiveDTO dto) {
        ...
        long rid = cache.getRid();
        LiveCache live = liveCacheComponent.getLive(cache.getRid());
        if (live != null ){
