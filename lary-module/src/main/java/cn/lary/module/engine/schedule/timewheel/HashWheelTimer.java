...
import cn.lary.module.engine.kit.CommonKit;
import cn.lary.module.engine.kit.RejectedExecutionHandlerFactory;
...
public class HashWheelTimer implements Timer {
    ...
    public HashWheelTimer(long tickDuration, int ticksPerWheel) {
     ...
     */
    public HashWheelTimer(long tickDuration, int ticksPerWheel, int processThreadNum) {

        this.tickDuration = tickDuration;

        // 初始化轮盘，大小格式化为2的N次，可以使用 & 代替取余
        int ticksNum = CommonKit.formatSize(ticksPerWheel);
    ...
    @Override
    public TimeFuture schedule(TimeTask task, long delay, TimeUnit unit) {
    ...
    private final class HashedWheelTimerFuture implements TimeFuture {
        ...
        public HashedWheelTimerFuture(TimeTask timerTask, long targetTime) {
    ...
    private class Indicator implements Runnable {
        ...
        public Set<TimeTask> getUnprocessedTasks() {
            try {
                latch.await();
            }catch (Exception ignore) {
            }

            Set<TimeTask> tasks = Sets.newHashSet();
